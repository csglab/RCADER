...
//////////////////////////////////////////////////////////
void convert_PWMs_to_likelihood(
	s_motif *motifs[],
	int num_motifs,
	double enrichment_fold )
{
	...
	{
		_convert_PWM_to_likelihood( motifs[ i ] ->PWM, motifs[ i ] ->PWM_width, enrichment_fold );
		if( motifs[ i ] -> PWM_optimized )
			_convert_PWM_to_likelihood( motifs[ i ] ->opt_PWM, motifs[ i ] ->PWM_width, enrichment_fold );
	}
}
	...
	int num_motifs )
{
	...
	for( i = 0; i < num_motifs; i ++ )
	{
		...
		int PWM_width;
		_get_PWM( motifs[ i ], PWM, &PWM_width );
		
		// store the PWM for this motif
		for( n = 0; n < NUM_N_LETTERS; n ++ )
		{
		...
		motifs[ i ] ->PWM_width = PWM_width - 1; // the actual motif is one nucleotide shorter than the RC output, because the last position is always 0
	}
