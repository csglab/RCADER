...
//
{
	...
	cout << *num_seqs << " sequences were read." << endl << endl;
	...
	int *num_seqs )
{
	...
	// read the file
	cout << "Reading the scores..." << endl;
	_CALL(
		read_scores( ifs, seqs, num_seqs, this_EOL )
		);

	if( *num_seqs >= 100 )
		cout << *num_seqs << " sequences had associated scores and were retained." << endl << endl;
	else
	{
		cout << "ERROR: Fewer than 100 sequences had associated scores." << endl;
		return false;
	}
